...
- In **Assisted Mode**, the 2bit PXLR Studio rom mimics the behavior of the Game Boy Camera, it modifies only the **Exposure time, gain and output reference voltage**. All other parameters used are fixed. In **Manual Mode**, the 2bit PXLR Studio rom allows **modifying all the parameters** (or registers) of the sensor. This could lead to unexpected results as certain parameter configurations are not expected to give a clear image or to be compatible with the scale of the ADC (analog to digital converter) of the camera.
